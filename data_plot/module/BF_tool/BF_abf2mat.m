function pfmats = BF_abf2mat(pfnames)
% 转化 .abf(Axon Binary File) 文件为 .mat。 可同时转化多个。
%function BF_abf2mat(pfnames)
%----Input 参数---------
% pfnames    : abf文件路径, 可省略
%
%----Output 参数---------
...
[p, f, ~] = fileparts(pf);
pfnew =  fullfile(p,[f, '_abf.mat']);
if BF_fileDateCmp(pf, pfnew)<0
    fprintf('> %s%s\t<Existed(skip)>\n\n', f, '.mat');
    return;
end
[data, si, h] = abfload(pf);
Fs = 1e6 / si; %采样率
time = (1:length(data))'/Fs;
chaninfo = struct();
chaninfo.channames = h.recChNames;
chaninfo.chanunits = h.recChUnits;
save(pfnew, 'time', 'data','chaninfo','Fs');
