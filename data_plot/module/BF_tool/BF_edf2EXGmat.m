...
if ~exist('pfnames', 'var')
    pfnames = uigetfilemult('*.edf');
end
for i=1:length(pfnames)
    trans(pfnames{i})
end

function trans(pfname)
%% parameter
gain = 0.001; %uV -> mV

%% load file
[hdr, record] = edfread(pfname);

%% check file
assert(size(record, 1)==4);
assert(isequal(hdr.label(1:3), {'EEGEEG1AB','EEGEEG2AB','EMGEMG'}));
assert(isscalar(unique(hdr.frequency(1:3))));
assert(isequal(unique(hdr.units(1:3)), {'uV'}));

%% assignment
MAT1 = struct();
MAT1.Fs = hdr.frequency(1);
MAT1.EMG = record(3, :)' * gain;

MAT2 = MAT1;
MAT1.EEG = record(1, :)' * gain;
MAT2.EEG = record(2, :)' * gain;

MAT3 = MAT1;
MAT3.EEG2 = MAT2.EEG;

%% save file
[p, f, ~] = fileparts(pfname);
f1 = fullfile(p, [f, '_EXG1.mat']);
save(f1, '-struct', 'MAT1');

f2 = fullfile(p, [f, '_EXG2.mat']);
save(f2, '-struct', 'MAT2');

f2 = fullfile(p, [f, '_EXG.mat']);
save(f2, '-struct', 'MAT3');

%% printf
fprintf('-------------------%s---------------\n', f);
fprintf('Fs   : %.0f\n', MAT1.Fs);
fprintf('Time : %s (sec)\n', char(duration(0,0,length(MAT1.EEG)/MAT1.Fs)));
fprintf('EEG1(%s), EEG2(%s), EMG(%s)\n', 'mV', 'mV', 'mV');

%% edf reader
% https://cn.mathworks.com/matlabcentral/fileexchange/31900-edfread
