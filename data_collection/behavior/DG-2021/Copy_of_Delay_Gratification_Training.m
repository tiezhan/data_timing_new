...
%      singleton*.
%
%      H = DELAY_GRATIFICATION_TRAINING returns the handle to a new DELAY_GRATIFICATION_TRAINING or the handle to
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[filename, pathname] = uiputfile('*.dat', 'Save CPPTest file');
if filename==0
    return;
end
...
handles.n = n;  %320
handles.m = m;  %240
position = [230   28  50  60];
position2=[70   26  50  60];
position3= [215   28  15  60];
...
if time > test_time||Water_Volume> 700 
    pushbutton_stop_Callback(hObject,eventdata,handles);
    Email('tiezhantest@163.com','timeup','Computer 2 task finished')
end
...
function edit_leftdistance_Callback(hObject, eventdata, handles)
